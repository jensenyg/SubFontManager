import os
from Global import AppInfo, Config
...
class LanguageDict:
    ...
    def __init__(self, langDir: str = None):
        self.langDir = langDir if langDir else os.path.join(AppInfo.getResourcesDirectory(), 'lang')
        self.lang = {}
        self.currentLang = self.ENGLISH
        self.initLang = Config.get('General', 'lang', 'English')
        self.langList = []
    ...
    def saveSetting(self, langName: str = None):
        if langName:
            self.currentLang = langName
        Config.set('General', 'lang', self.currentLang)
