"""来自matplotlib的findSystemFonts函数"""
                ...
                try:
                    with winreg.OpenKey(domain, reg_path) as local:
                        for j in range(winreg.QueryInfoKey(local)[1]):
                            # value may contain the filename of the font or its absolute path.
                            key, value, tp = winreg.EnumValue(local, j)
                            if not isinstance(value, str):
                                continue
                            try:
                                # If value contains already an absolute path, then it is not changed further.
...
def _get_fontconfig_fonts():
    ...
    使用外部包fontconfig提供的命令fc-list来获取系统字体，该方法执行很快但需要系统中已安装fontconfig
    """
    try:
        if b'--format' not in subprocess.check_output(['fc-list', '--help']):
            print('Requires fontconfig>=2.7 to query system fonts.')
            return []
    ...
    if fontpaths is None:
        if sys.platform == 'win32':
            ...
            if sys.platform == 'darwin':
                # installed_fonts += _get_macos_fonts()    # 该函数执行很慢，非必要不启用
