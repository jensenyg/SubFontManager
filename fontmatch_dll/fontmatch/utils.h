...
// 字体的指定名字类型中是否包含指定的名字
bool FontHasName(Microsoft::WRL::ComPtr<IDWriteFont> font, DWRITE_INFORMATIONAL_STRING_ID stringId, std::wstring& name);
...
T3 GetEnumFromMap(T<T1, T2, Rest...>& _map, T1 key, T3 _default, bool* exists = nullptr) {
    auto it = _map.find(key);
    if (exists)
        *exists = it != _map.end();
