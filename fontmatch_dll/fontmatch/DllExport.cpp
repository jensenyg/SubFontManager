...
bool Init() {   // 初始化
    ...
    if (FontMatch::IsSupported()) {
        if (FontMatch3::IsSupported()) {    // 支持DW3，初始化FontMatch3
            if (fontMatcher3.Init())
                fontMatcher = &fontMatcher3;
            else
                return false;
        }
        else {  // 仅支持DW1，初始化FontMatch
...
const char* GetMatchingFont(const char* json_str) {
    // 解析JSON串 ------
    nlohmann::json json;
    try {
    ...
    path_str = utf8conv.to_bytes(fontMatcher->GetMatchingFont(prop_dict));
