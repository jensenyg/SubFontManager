...
Sub Font Manager 用于编辑.ASS或.SSA字幕文件中的字体设置，具体来说，
它可以将字幕中所有使用过的字体汇总，并可以将字体按照字幕所覆盖的字符集进行子集化并嵌入到字幕文件中。
...
不过，SSA/ASS 规定使用 UUEncoding 编码将字体文件编码为文本保存，对于非西方语系的语言来说，字体文件本来就大，
...
- 汇总出字幕中所有使用过的字体（包括 Style 中的设置和行内的设置）；
- 在同目录和系统中自动匹配字体源文件；
- 收集字体在字幕中覆盖的字符集；
...
部分字幕编辑器对字幕附件兼容性不好，它们可能会忽略附件并在保存时删掉它们。
