...
class FontList(ui.WidgetTable):
    ...
    def __init__(self, master):
        ...
        # 字体名列
        weight = float(App.Config.get('General', 'fontColumnWeight', 1))
        ...
        # 字数列
        width = int(App.Config.get('General', 'countColumnWeight', 55))
        ...
        # 文件源列
        weight = float(App.Config.get('General', 'sourceColumnWeight', 2))
    ...
    def loadSubtitle(self, subObj: SubStationAlpha = None):
        ...
        for fontDesc in subFontDescs:
            ...
            if row_item.isEmbed:  # 如果是内嵌字体
                row_item.embed.set(True)    # 勾上内嵌复选框
    ...
    def checkTaskValidity(self) -> bool:
        """检查表中任务设置是否正确可执行，必要时弹窗询问，同时会给每一行找到新字体源并记录"""
        warnings: list[str] = []  # 警告文本列表
        have_task = False  # 标记是否有任务要执行
        row_items = [row.data for row in self._rows if not row.isSep]   # 去掉分割行，获取所有的行信息

        # 检查是否有可做的任务、文件是否存在，以及文件内是否都包含指定的字体 -------------
        for row_item in row_items:
            # 检查是否有可做的任务 -------------
            if row_item.embed.get():
                if row_item.isEmbed and not row_item.subset.get():
                    continue
                else:  # 将外部字体内嵌，或者将原内嵌字体再次子集化
                    have_task = True
            else:  # 未勾选的字体不用检查
                if row_item.isEmbed:  # 原内嵌字体未勾选内嵌，意味着要删除内嵌
                    have_task = True
                continue
    ...
    def applyEmbedding(self, savePath: str = None):
        ...
        embed_fonts: dict[str, EmbedFont] = {}  # 按Postscript名索引待内嵌字体，用于合并重复的字体

        for row_item in row_items:
            font = row_item.font
            if row_item.embed.get():    # 勾选了内嵌的行
                if not font.isInMemory: # 外部文件字体源
                    ...
                    while embed_name + '.ttf' in self.subtitleObj.fontDict:
                        ...
                        i += 1
                    embed_name += '.ttf'
                elif not row_item.subset.get():  # 内嵌字体源，且不需要子集化的字体，直接跳过
                    continue
                else:   # 内嵌字体源，需要子集化
                ...
                if font.postscriptName in embed_fonts:  # 如果字体已存在，合并多次引用
                    ...
                    embed_fonts[font.postscriptName] =\
                        EmbedFont(embed_name, [row_item.fontName.lower()],
                                  row_item.text, row_item.subset.get(), font)
            # 原内嵌字体未勾选内嵌，意味着要删除内嵌。内嵌字体可能是多个行的文件源，所以该字体可能已经被删除过了，因此需要检查一下
            elif row_item.isEmbed and font.path in self.subtitleObj.fontDict:
                font_codes = self.subtitleObj.fontDict[font.path]
                if len(font_codes) == 1:
